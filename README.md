...
1. **Easysearch**（开启 kNN 插件 + ES API 兼容）
   ...
   - `easysearch.yml`：
     ...
     ```
   - 保留 HTTP 或 HTTPS + basic auth（默认 `admin` / `<密码>`）
2. **Maven Central 可访问**
...
easysearch:
  host: localhost
  port: 9200
  scheme: ${EASYSEARCH_SCHEME:https}   # 支持 http / https，默认 https
  username: admin
...
```bash
EASYSEARCH_SCHEME=https \
EASYSEARCH_PASSWORD='<你的密码>' \
mvn spring-boot:run
```

启动后自动写入 6 条新闻标题样例文档，监听 `8080`。

如果你的 Easysearch 暴露的是明文 HTTP：
...
如果你的 Easysearch 是本地明文 HTTP 服务，例如 `http://localhost:9201`，可以直接用下面这条完整命令：
...
脚本同样会继承 `EASYSEARCH_HOST`、`EASYSEARCH_PORT`、`EASYSEARCH_SCHEME`、`EASYSEARCH_PASSWORD`
这些环境变量。如果你的 Easysearch 是本地明文 HTTP 服务 `http://localhost:9201`，可以这样跑：
...
以下示例默认应用监听 `http://localhost:8080`。如果你启动时设置了 `SERVER_PORT`，
请把下面所有 URL 里的 `8080` 同步替换掉。
...
当前 demo 同时支持 `http` 和 `https`，通过 `easysearch.scheme` / `EASYSEARCH_SCHEME`
控制，默认值是 `https`。
...
说明客户端按 `https` 去连了一个实际只提供 `http` 的端口。把 `EASYSEARCH_SCHEME=http`
传进去，或把 `easysearch.scheme` 改成 `http`。
